...
def main() -> int:
    ...
    out_file = Path(_env("OUT_FILE", "d:/connect casa os/tools/_services_restart.txt"))
    ...
    steps = [
        ...
        ("containerd_link", "bash -lc \"ls -l /var/lib/containerd || true; readlink -f /var/lib/containerd || true\""),
        (
            "kasir_up_no_build",
            "bash -lc \"export TERM=dumb; cd "
            + kasir_root
            + " && (docker compose up -d || docker-compose up -d) > /tmp/kasir_up.log 2>&1; echo __EXIT:$?; tail -n 60 /tmp/kasir_up.log\"",
        ),
        (
            "chat_up_build",
            "bash -lc \"export TERM=dumb; cd "
            + chat_root
            + " && (docker compose up -d --build || docker-compose up -d --build) > /tmp/chat_up.log 2>&1; echo __EXIT:$?; tail -n 80 /tmp/chat_up.log\"",
        ),
        (
            "ollama_pull",
            "bash -lc \"export TERM=dumb; docker exec connect-ai-chat-ollama ollama pull "
            + model
            + " > /tmp/ollama_pull.log 2>&1; echo __EXIT:$?; tail -n 60 /tmp/ollama_pull.log\"",
        ),
        (
            "ports",
